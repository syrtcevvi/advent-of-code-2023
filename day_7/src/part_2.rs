...
impl Hand {
    ...
    fn determine_kind(cards: &[Card]) -> HandKind {
        ...
        let jokers_quantity = card_kinds.get(&Card::Jocker).cloned().unwrap_or_default();
        ...
        match counters.as_slice() {
            ...
            [four, ..] if *four == 4 => {
                if jokers_quantity == 1 || jokers_quantity == 4 {
                    HandKind::FiveOfAKind
                } else {
                    HandKind::FourOfAKind
                }
            }
            [three, _middle @ .., two] if *three == 3 && *two == 2 => match jokers_quantity {
                2 | 3 => HandKind::FiveOfAKind,
                _ => HandKind::FullHouse,
            },
            rest => {
                if rest == [3, 1, 1] {
                    if jokers_quantity == 1 || jokers_quantity == 3 {
                        HandKind::FourOfAKind
                    } else {
                        HandKind::ThreeOfAKind
                    }
                } else if rest == [2, 2, 1] {
                    match jokers_quantity {
                        1 => HandKind::FullHouse,
                        2 => HandKind::FourOfAKind,
                        _ => HandKind::TwoPair,
                    }
                } else if rest == [2, 1, 1, 1] {
                    if jokers_quantity == 1 || jokers_quantity == 2 {
                        HandKind::ThreeOfAKind
                    } else {
                        HandKind::OnePair
                    }
                } else if jokers_quantity == 1 {
                    HandKind::OnePair
                } else {
                    HandKind::HighCard
                }
